...
def test_fetch_public_growth_snapshot_uses_github_public_endpoints_and_excludes_prs():
    ...
    responses = {
        ...
        "https://api.github.com/repos/octo-org/octo-repo/releases/tags/v0.1.15": {
            "tag_name": "v0.1.15",
            "name": "TianGong 0.1.15",
            "html_url": "https://github.com/octo-org/octo-repo/releases/tag/v0.1.15",
            "draft": False,
            "prerelease": False,
    ...
    assert snapshot.release_readiness.local_version == "0.1.15"
    assert snapshot.release_readiness.expected_tag == "v0.1.15"
    assert snapshot.release_readiness.status == "published"
    assert snapshot.release_readiness.html_url == "https://github.com/octo-org/octo-repo/releases/tag/v0.1.15"
    assert snapshot.distribution_readiness.package_name == "tiangong-mcp"
    assert snapshot.distribution_readiness.local_version == "0.1.15"
    assert snapshot.distribution_readiness.published_version == "0.1.15"
    ...
    assert any(call[0] == "https://api.github.com/repos/octo-org/octo-repo/releases/tags/v0.1.15" for call in calls)
...
def test_public_growth_report_prints_not_closed_flywheel_verdict(tmp_path):
    ...
    assert "| GitHub Release trigger | blocked | `v0.1.15` is missing" in result
    assert "| PyPI install loop | blocked | PyPI latest `0.0.1` vs local `0.1.15`" in result
...
def test_public_growth_report_gives_first_public_proof_action_for_cold_start(tmp_path):
    ...
    assert (
        'Install: python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/octo-org/octo-repo.git@v0.1.15"'
        in result
    )
...
def test_public_install_command_bridges_stale_pypi_distribution():
    ...
    assert "| `tiangong-mcp` | `0.1.15` | `0.0.1` | stale |" in result
    assert "## Current Candidate Git Tag Install Bridge" in result
    assert 'python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/octo-org/octo-repo.git@v0.1.15"' in result
...
def test_public_launch_preflight_fetch_failure_includes_release_fallbacks(tmp_path):
    ...
    assert (
        ...
        "`JinNing6/TianGong/.github/workflows/publish-pypi.yml@refs/tags/v0.1.15` |"
    ) in result
    assert (
    ...
    assert "tiangong-mcp public-proof-pack --target-contributors 10" in result
    assert "## Current Candidate Git Tag Install Bridge" in result
    assert 'python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/JinNing6/TianGong.git@v0.1.15"' in result
    assert "Canonical install after PyPI latest is current: `pip install -U tiangong-mcp`" in result
    assert "does not close the PyPI install loop" in result
