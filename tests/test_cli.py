    ...
    *,
    ...
    workflow_text = "\n".join(
        [
            ...
            "  contents: read",
            "steps:",
            '  - run: python .github/scripts/run_pytest_with_annotations.py',
        ...
        for module in required_modules:
            if module == "tiangong/cli.py" and include_terminal_ledger_commands:
                ...
                archive.writestr(
                    ...
                    "\n".join(
                        [
                            "from .achievement_card import format_achievement_card",
                            "@mcp.tool()",
                            "async def achievement_card():",
                            "    return format_achievement_card(profile)",
                            "@mcp.tool()",
...
def test_cli_public_launch_preflight_prints_ordered_release_runbook(monkeypatch, tmp_path):
    ...
    assert "gh release create v0.1.19 --generate-notes" in output
    assert "https://github.com/octo-org/octo-repo/releases/new" in output
    assert "Select existing tag `v0.1.19`" in output
...
def test_cli_public_launch_preflight_inlines_full_release_handoff(monkeypatch, tmp_path):
    ...
    assert "git add tiangong/achievement_card.py" in output
    assert "tiangong/activation.py" in output
    assert "git add tests/test_achievement_card.py" in output
    assert "tests/test_cli.py" in output
    assert 'git commit -m "Prepare TianGong public growth launch"' in output
    assert "git push origin HEAD:main" in output
    assert "git push origin main" not in output
    assert "gh release create v0.1.19 --generate-notes" in output
    assert "git push origin v0.1.19" in output
...
def test_cli_public_launch_assets_prints_local_push_manifest():
    ...
    assert "CI pytest helper: ready" in output
    ...
    assert ".github/scripts/run_pytest_with_annotations.py" in output
...
def test_cli_public_install_command_prints_current_candidate_bridge(monkeypatch):
    ...
    assert 'python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/JinNing6/TianGong.git@v0.1.19"' in output
...
def test_cli_public_candidate_smoke_formats_real_install_gate(monkeypatch, tmp_path):
    ...
    def fake_smoke(**kwargs):
        ...
        return CandidateSmokeResult(
            ...
            repo_name="octo-repo",
            tag="v0.1.19",
            requirement="tiangong-mcp @ git+https://github.com/octo-org/octo-repo.git@v0.1.19",
            install_command='python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/octo-org/octo-repo.git@v0.1.19"',
    ...
    assert calls[0]["version_or_tag"] == "v0.1.19"
    assert calls[0]["target_contributors"] == 25
    assert "# TianGong Public Candidate Install Smoke" in output
    assert "- Status: ready" in output
    assert "| Installed package version | ready | `0.1.19` matches `v0.1.19` |" in output
...
def test_candidate_smoke_runner_captures_subprocess_output_as_utf8(tmp_path):
    ...
    def fake_runner(command, **kwargs):
        ...
        if "-m venv" in text_command:
            ...
            (bin_dir / "tiangong-mcp").write_text("", encoding="utf-8")
            return subprocess.CompletedProcess(command, 0, "", "")
        if "importlib.metadata" in text_command:
            return subprocess.CompletedProcess(command, 0, "0.1.19\n", "")
        if "public-install-command" in text_command:
            return subprocess.CompletedProcess(command, 0, "candidate v0.1.19 天工\n", "")
        if "public-proof-pack" in text_command:
            return subprocess.CompletedProcess(command, 0, "v0.1.19\nCopy External Contributor Invite\n", "")
        return subprocess.CompletedProcess(command, 0, "", "")

    result = run_public_candidate_install_smoke(
        repo_owner="octo-org",
        repo_name="octo-repo",
        version_or_tag="v0.1.19",
...
def test_cli_public_proof_pack_prints_no_network_first_proof_runbook():
    ...
    assert 'python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/octo-org/octo-repo.git@v0.1.19"' in output
...
def test_cli_public_release_boundary_prefers_current_version_artifacts(tmp_path):
    ...
    for relative in ["README.md", "README.zh-CN.md", ".github/workflows/issueops-onboarding.yml"]:
        ...
        path.write_text(path.read_text(encoding="utf-8").replace("@v0.1.0", "@v0.1.19"), encoding="utf-8")

    stale_wheel = dist_dir / "tiangong_mcp-0.1.0-py3-none-any.whl"
    current_wheel = dist_dir / "tiangong_mcp-0.1.19-py3-none-any.whl"
    with zipfile.ZipFile(stale_wheel) as source, zipfile.ZipFile(current_wheel, "w") as target:
        for name in source.namelist():
            if ".dist-info/" not in name:
                target.writestr(name, source.read(name))
        target.writestr(
            "tiangong_mcp-0.1.19.dist-info/METADATA",
            "\n".join(
                [
                    "Metadata-Version: 2.1",
                    "Name: tiangong-mcp",
                    ...
                    "Description-Content-Type: text/markdown",
                    "",
                    'Current candidate install: python -m pip install --upgrade "tiangong-mcp @ git+https://github.com/JinNing6/TianGong.git@v0.1.19"',
                    "",
                ]
            ),
        )
        target.writestr(
            "tiangong_mcp-0.1.19.dist-info/entry_points.txt",
            "[console_scripts]\ntiangong-mcp = tiangong.cli:main\n",
        )

    stale_sdist = dist_dir / "tiangong_mcp-0.1.0.tar.gz"
    current_sdist = dist_dir / "tiangong_mcp-0.1.19.tar.gz"
    with tarfile.open(stale_sdist, "r:gz") as source, tarfile.open(current_sdist, "w:gz") as target:
        for member in source.getmembers():
            extracted = source.extractfile(member)
            if extracted is None:
                continue
            member.name = member.name.replace("tiangong_mcp-0.1.0/", "tiangong_mcp-0.1.19/", 1)
            if member.name.endswith("/README.md"):
                data = source.extractfile(member)
                if data is None:
                    continue
                readme_text = data.read().decode("utf-8").replace("@v0.1.0", "@v0.1.19")
    ...
    assert "tiangong_mcp-0.1.19-py3-none-any.whl" in output
    assert "tiangong_mcp-0.1.0-py3-none-any.whl` version `0.1.0` vs local `0.1.19" not in output
    assert "Local Release Boundary Status" in output
